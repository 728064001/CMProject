...
$(function(){
	...
	$("#aqq").blur(function(){
		...
		var aqqReg=/^\d*$/;
		var js_add_queryqqservletstr = "/CMS/registerQueryQqServlet?"+"&qq="+aqqStr;
		if(aqqlen>=4&&aqqlen<=15){
			aajaxreturn(js_add_queryqqservletstr,$("#aqq"),aqqtipobj);
...
$("#atel").blur(function(){
	...
	var atelReg=/^\d*$/;
	var js_add_querytelservletstr = "/CMS/registerQueryTelServlet?"+"&tel="+atelStr;
	if(atellen>=7&&atellen<=12){
		if(atelReg.test(atelStr)){
			aajaxreturn(js_add_querytelservletstr,$("#atel"),ateltipobj);
...
$("#acard").blur(function(){
	 ...
	 var js_add_querycardservletstr = "/CMS/registerQueryCardServlet?"+"&card="+acardStr;
	 if(acardReg.test(acardStr)){
	    aajaxreturn(js_add_querycardservletstr,$("#acard"),acardtipobj);
...
function addpersonvalidata(){
			...
			if(!aqqReg.test(aqqStr)){
				...
				$(function(){
					$.ajax({
						...
						async:false, 
						dataType:'json',
						success:function(aqqjson) {
							if(aqqjson.status=="yes"){
								a_qq_flag = true;
								//set style 
								$("#aqq").css("border-color","#0000FF","border-width","2px","border-style","solid");
								aqqtipobj.html("OK");
								aqqtipobj.css("color","#0000FF");
							}else{
								a_qq_flag = false;
								$("#aqq").css("border-color","red","border-width","2px","border-style","solid");
								aqqtipobj.html("该号码已经被注册!");
								aqqtipobj.css("color","red");
							}
						}
					});
	...
	var ateltipobj = $("#ateltip");
	var atelReg=/^\d*$/;
	var add_querytelservletstr = "/CMS/registerQueryTelServlet?"+"&tel="+atelStr;
	if(atellen<7||atellen>12){
		a_tel_flag = false;
		$("#atel").css("border-color","red","border-width","2px","border-style","solid");
		ateltipobj.html("电话号码长度错误!(7-12位)!");
...
//			}
			if(!atelReg.test(atelStr)){
				...
				$(function(){
					$.ajax({
						url: add_querytelservletstr,
						type: "POST",
						async:false, 
						dataType:'json',
						success:function(ateljson) {
							if(ateljson.status=="yes"){
								a_tel_flag = true;
								$("#atel").css("border-color","#0000FF","border-width","2px","border-style","solid");
								ateltipobj.html("OK");
								ateltipobj.css("color","#0000FF");
							}else{
								a_tel_flag = false;
								$("#atel").css("border-color","red","border-width","2px","border-style","solid");
								ateltipobj.html("该号码已经被注册!");
								ateltipobj.css("color","red");
							}
						}
					});
